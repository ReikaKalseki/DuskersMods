...
namespace ReikaKalseki.DIDrones {
	...
	{		
		...
		public static CodeInstruction createMethodCall(string owner, string name, params string[] args) {
			return new CodeInstruction(OpCodes.Call, convertMethodOperand(owner, name, args));
		}
		
		public static CodeInstruction createMethodCall(string owner, string name, params Type[] args) {
			return new CodeInstruction(OpCodes.Call, convertMethodOperand(owner, name, args));
		}
		
		public static MethodInfo convertMethodOperand(string owner, string name, params string[] args) {
			Type[] types = new Type[args.Length];
			for (int i = 0; i < args.Length; i++) {
				types[i] = AccessTools.TypeByName(args[i]);
			}
			return convertMethodOperand(owner, name, types);
		}
		
		public static MethodInfo convertMethodOperand(string owner, string name, params Type[] args) {
		...
		public static int getInstruction(List<CodeInstruction> li, int start, int index, OpCode opcode, params object[] args) {
			...
			throw new Exception("Instruction not found: "+opcode+" #"+args.toDebugString()+"\nInstruction list:\n"+toString(li));
		}
		...
		public static bool match(CodeInstruction insn, params object[] args) {
			...
			if (insn.opcode == OpCodes.Call || insn.opcode == OpCodes.Callvirt) { //string class, string name, bool instance, Type[] args
				MethodInfo info = convertMethodOperand((string)args[0], (string)args[1], (Type[])args[2]);
		...
		private static string toOperandString(OpCode code, object operand) {
			if (operand is MethodInfo) {
				...
				return m.DeclaringType+"."+m.Name+" ("+string.Join(", ", m.GetParameters().Select<ParameterInfo, string>(p => p.ParameterType.Name+" "+p.Name).ToArray()) +") [static="+m.IsStatic+"]";
			}
		...
		public static void runPatchesIn(Harmony h, Type parent) {
			...
			DSUtil.log("Running harmony patches in "+parent.Assembly.GetName().Name+"::"+parent.Name);
